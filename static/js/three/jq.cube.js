...
$.fn.cube = function(options) {
	...
	function base64FaceTexture(width, height, base64) {
		//optimize texture creation
		if(colorFaceTexture[base64])
			return colorFaceTexture[base64];
		...
		image0.onload = function () {
			...
			image.src = canvas0.toDataURL();
			console.log('wenli', image.src)
		}
		image0.src = base64;

		colorFaceTexture[base64] = texture;
	...
	function createCubit(point){
		...
		for(var i=0; i<6; i++){
			...
			var texture
			if (typeof(color) == 'string') { // base64
				// 纹理
				texture = base64FaceTexture(32, 32, color);
			} else {
				// 纯色
				texture = colorFaceTexture(32, 32, color);
			}
	...
	function createCube(){
		...
		options.cubit = {
			...
			length: options.size.length/options.type,
		...
		for(var iz=0; iz<options.type; iz++){
			for(var iy=0; iy<options.type; iy++){
				for(var ix=0; ix<options.type; ix++){
					...
					point.y = options.cubit.length * (iy - run) + offset;
	...
	// create the mirror
	var mirror = null, mirror2 = null, mirror3 = null;
	_ref.createMirror = function() {
		// 镜子形状
		let planeGeometry = new THREE.PlaneBufferGeometry(options.size.width*1.5, options.size.height*1.5);
		// 配置
		options.mirror = {
			clipBias: 0.03,
			textureWidth: _ref.width() * window.devicePixelRatio,
			textureHeight: _ref.height() * window.devicePixelRatio,
			color: 0x000000,//0x889999,
			// recursion: 1
		};

		if (mirror == null) {
			mirror = new THREE.Reflector(planeGeometry, options.mirror);
			mirror.rotation.y += 0.5*Math.PI;
			mirror.position.x = -options.size.width * 1.6;
			mirror.position.y = planeGeometry.parameters.height / 2 - 20;
			mirror.position.z = planeGeometry.parameters.width / 2 - 20;
		}
		_scene.add(mirror);

		if (mirror2 == null) {
			mirror2 = new THREE.Reflector(planeGeometry, options.mirror);
			mirror2.position.z = -options.size.width * 1.6;
			mirror2.position.y = planeGeometry.parameters.height / 2 - 20;
			mirror2.position.x = planeGeometry.parameters.width / 2 - 20;
		}
		_scene.add(mirror2);

		if (mirror3 == null) {
			mirror3 = new THREE.Reflector(planeGeometry, options.mirror);
			mirror3.rotation.x -= 0.5*Math.PI;
			mirror3.position.y = -options.size.width * 1.6;
			mirror3.position.x = planeGeometry.parameters.height / 2 - 20;
			mirror3.position.z = planeGeometry.parameters.width / 2 - 20;
		}
		_scene.add(mirror3);
	}
	_ref.removeMirror = function() {
		if (mirror != null) _scene.remove(mirror);
		if (mirror2 != null) _scene.remove(mirror2);
		if (mirror3 != null) _scene.remove(mirror3);
	}
	...
	function constructor() {
		...
		// _camera = new THREE.OrthographicCamera(-_ref.width(), _ref.width(), _ref.height(), -_ref.height(), 0.1, 1000); // 正交相机
		_camera = new THREE.PerspectiveCamera(75, _ref.width() / _ref.height(), 0.1, 1000); // 透视相机 Reflector只支持透视相机
